...
quadConfig = QuadrotorModel( ...
    ...
    "MaxThrustFactor", 5);           % Tmax = fator*m*g
...
%   "YawDesired"           -> yaw desejado constante da trajetoria [rad].
%   "YawMode"              -> "cons" ou "target".
%   "YawTargetXY"          -> ponto [x y] para o qual o yaw deve apontar
%                             quando "YawMode" = "targetYaw".
%   "IntegrationMethod"    -> "RK4" ou "Euler".
%   "InitialState"         -> vetor de estado inicial 12x1.
%   "ResetStateEachLap"    -> reinicia ou nao o estado no inicio de cada volta.
%
% Conceitos:
...
noiseParams.eta.enable   = false;    % phi, theta, psi
noiseParams.omega.enable = false;   % p, q, r
...
% ========================================================================
% Modelos disponiveis:
%   "Normal"    -> controle atual: posicao P/PD/PID + atitude P/PD/PID
%   "MRAC_Test" -> x/y P/PD/PID + altitude MRAC + atitude MRAC
%
% Para trocar o modelo, altere apenas a variavel simulationModel.

%simulationModel = "Normal";
...
fprintf('\n--- Controladores ---\n');
fprintf('Modelo de simulacao: %s\n', char(controllersConfig.simulationModel));

if simulationModel == "Normal"
...
if simulationModel == "MRAC_Test"
    fprintf('x/y: %s @ %.2f Hz\n', controllersConfig.position.type, controllersConfig.position.updateFrequency);
    fprintf('Altitude: %s @ %.2f Hz\n', controllersConfig.altitude.type, controllersConfig.altitude.updateFrequency);
    fprintf('Atitude: %s @ %.2f Hz\n', controllersConfig.attitude.type, controllersConfig.attitude.updateFrequency);
end
